...
export const actions = {
	register: async ({ request, params }) => {
		...
		if (!formImage || !(formImage instanceof File)) {
			...
			formImage.type !== 'image/jpeg' &&
			formImage.type !== 'image/png' &&
			formImage.type !== 'image/webp'
		) {
		...
		try {
			const image = await uploadImage(formImage as File, {
				asset_folder: 'user',
				public_id: `client/${params.client_id}/dog/${randomUUID()}`
			});
			if (!image.success) {
